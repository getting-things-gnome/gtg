...
def dsanitize(data):
  ...
  for k, v in data.items():
    ...
    if not v and isinstance(v, list):
      data[k] = dbus.Array([], "s")
    # D-Bus has no concept of a null or empty value so we have to convert None
    # types to something else. I use an empty string because it has the same
    # behavior as None in a Python conditional expression
    elif v == None:
      data[k] = ""
