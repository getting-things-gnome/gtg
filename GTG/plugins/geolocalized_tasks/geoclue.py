...
class Geoclue:
    ...
    def __init__(self):
        ...
        manager_object = system_bus.get_object(
            self.GEOCLUE2_BUS_NAME,
            '/org/freedesktop/GeoClue2/Manager'
        )
        manager = dbus.Interface(manager_object, self.MANAGER_INTERFACE_NAME)

        client_path = manager.GetClient()
        client_object = system_bus.get_object(
            self.GEOCLUE2_BUS_NAME,
            client_path
        )
        client_properties = dbus.Interface(
            client_object,
            self.PROPERTIES_INTERFACE_NAME
        )
