...
#we can store
def serialize(register_buf, content_buf, start, end, udata) :
...
def parse_buffer(buf, start, end, name, doc,tag_stack) :
    """
        ...
        if it.begins_tag() :
            ...
            if all_processed :
                ...
                if ta.get_data('is_subtask') :
                    ...
                    child = parse_buffer(buf,startit,endit,"tag",doc,tag_stack)
                    parent.appendChild(child)
                else :
                    #The link tag has noname but has "is_anchor" properties
                    if ta.get_data('is_anchor'): tagname = "link"
                    #Recursive call !!!!! (we handle tag in tags)
                    child = parse_buffer(buf,startit,endit,tagname,doc,tag_stack)
